...
# Original script by Roger Heslop, Red Hat
# Adapted by Lena Ansorgova for Satellite Docs
...
def link_updater(regex_match):
    ...
    print(Highlighter(f"Fix found! moving {regex_match.group(0)} to {file_path}#{regex_match.group(1)}{regex_match.group(2)}").bold())
    return(f"{file_path}#{regex_match.group(1)}{regex_match.group(2)}")
